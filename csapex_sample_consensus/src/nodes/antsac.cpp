...
public:
    ...
    void setupParameters(Parameterizable& parameters) override
    {
        ...
        parameters.addParameter(param::factory::declareBool("use outlier probability", false), antsac_parameters_.use_outlier_probability);
        parameters.addConditionalParameter(param::factory::declareRange("outlier probability", 0.01, 1.0, 0.9, 0.01), [this]() { return antsac_parameters_.use_outlier_probability; },
                                           antsac_parameters_.outlier_probability);

        parameters.addParameter(param::factory::declareValue("random seed", -1), std::bind(&Antsac::setupRandomGenerator, this));
        parameters.addParameter(param::factory::declareRange("maximum sampling retries", 1, 1000, 100, 1), antsac_parameters_.maximum_sampling_retries);

        parameters.addParameter(param::factory::declareRange("rho", 0.0, 1.0, 0.9, 0.01), antsac_parameters_.rho);
        parameters.addParameter(param::factory::declareRange("alpha", 0.01, 10.0, 0.1, 0.01), antsac_parameters_.alpha);
    }
    ...
    void inputCloud(typename pcl::PointCloud<PointT>::ConstPtr cloud)
    {
        ...
        typename csapex_sample_consensus::Antsac<PointT>::Ptr sac(new csapex_sample_consensus::Antsac<PointT>(indices, antsac_parameters_, rng_));
