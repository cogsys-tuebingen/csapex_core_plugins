...
public:
    ...
    Ransac(const std::vector<int>& indices, const RansacParameters& parameters, std::default_random_engine& rng)
      : Base(indices), parameters_(parameters), rng_(rng), distribution_(0, Base::indices_.size() - 1), one_over_indices_(1.0 / static_cast<double>(Base::indices_.size()))
    {
    }
    ...
    virtual bool computeModel(typename Model::Ptr& model) override
    {
        ...
        while (!termination()) {
            ...
            if (parameters_.model_validation_ratio == 0.0 || parameters_.model_validation_ratio == 1.0) {
...
protected:
    struct InternalParameters
    {
        ...
        InternalParameters(const RansacParameters& params, const std::size_t model_dimension)
    ...
    inline void drawSamples(const std::size_t samples, std::vector<int>& indices)
    {
        std::size_t size = 0;
        std::set<int> selection;
