...
ParallelNormalPlane<PointT, NormalT>::ParallelNormalPlane(const typename PointCloud::ConstPtr& pointcloud, const typename NormalCloud::ConstPtr& normalcloud, const float normal_distance_weight)
  : Base(pointcloud, normalcloud, normal_distance_weight), angle_eps_(0.f)
{
}
...
{
    ParallelNormalPlane<PointT, NormalT>* plane = new ParallelNormalPlane<PointT, NormalT>(Base::pointcloud_, Base::normalcloud_, Base::normal_distance_weight_);
...
template <typename PointT, typename NormalT>
inline void ParallelNormalPlane<PointT, NormalT>::setAxis(const NormalT& axis, const float angle_eps)
{
    axis_ = axis;
...
{
    return std::abs(Base::model_coefficients_[0] * axis_.normal_x + Base::model_coefficients_[1] * axis_.normal_y + Base::model_coefficients_[2] * axis_.normal_z);
}
}  // namespace models
}  // namespace csapex_sample_consensus
