...
EvaluateFeatureClassification::BinaryClassificationMetrics::BinaryClassificationMetrics(BinaryClassificationResult result)
  ...
  , mcc((result.tp * result.tn - result.fp * result.fn) / std::sqrt(double(result.tp + result.fp) * double(result.tp + result.fn) * double(result.tn + result.fp) * double(result.tn + result.fn)))
{
}
