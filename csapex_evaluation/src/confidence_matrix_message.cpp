...
ConfidenceMatrixMessage::ConfidenceMatrixMessage(Message::Stamp stamp)
    : Message(type<ConfidenceMatrixMessage>::name(), "/", stamp)
{}

ConnectionType::Ptr ConfidenceMatrixMessage::clone() const
{
...
ConnectionType::Ptr ConfidenceMatrixMessage::toType() const
{
    Ptr new_msg(new ConfidenceMatrixMessage);
    return new_msg;
}
...
Node convert<csapex::connection_types::ConfidenceMatrixMessage>::encode(const csapex::connection_types::ConfidenceMatrixMessage& rhs)
{
    ...
    for(std::vector<int>::const_iterator class_it = rhs.confidence.classes.begin();
        class_it != rhs.confidence.classes.end(); ++class_it) {
        node["class_set"].push_back(*class_it);
    }

    for(std::vector<int>::const_iterator actual_it = rhs.confidence.classes.begin();
        actual_it != rhs.confidence.classes.end(); ++actual_it) {

        for(std::vector<int>::const_iterator predicted_it = rhs.confidence.classes.begin();
            predicted_it != rhs.confidence.classes.end(); ++predicted_it) {
            float value = 0.f;
            int   ups   = 0;
            try {
