...
ConfusionMatrixMessage::ConfusionMatrixMessage(Message::Stamp stamp) : Message(type<ConfusionMatrixMessage>::name(), "/", stamp)
{
}
...
Node convert<csapex::connection_types::ConfusionMatrixMessage>::encode(const csapex::connection_types::ConfusionMatrixMessage& rhs)
{
    ...
    for (std::vector<int>::const_iterator class_it = rhs.confusion.classes.begin(); class_it != rhs.confusion.classes.end(); ++class_it) {
        node["class_set"].push_back(*class_it);
    }

    for (std::vector<int>::const_iterator actual_it = rhs.confusion.classes.begin(); actual_it != rhs.confusion.classes.end(); ++actual_it) {
        for (std::vector<int>::const_iterator predicted_it = rhs.confusion.classes.begin(); predicted_it != rhs.confusion.classes.end(); ++predicted_it) {
            int value = 0;
            try {
