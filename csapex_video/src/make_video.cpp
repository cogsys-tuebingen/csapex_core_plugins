...
void MakeVideo::setupParameters(Parameterizable& parameters)
{
    ...
    file_extensions_ = {
        ...
        { codec_types["MPEG-4"], ".mpg" },       { codec_types["MPEG-4.2"], ".mpg" }, { codec_types["MPEG-4.3"], ".mpg" },    { codec_types["XVID"], ".avi" },
    };

    parameters.addParameter(param::factory::declareParameterSet("codec", codec_types, 0), codec_type_);
    parameters.addParameter(param::factory::declareRange("fps", 0.1, 100.0, 33.0, 0.1), frame_rate_);
    parameters.addParameter(param::factory::declareRange("width", 0, 1920, 0, 1), frame_size_.width);
    parameters.addParameter(param::factory::declareRange("height", 0, 1080, 0, 1), frame_size_.height);
    parameters.addParameter(param::factory::declareBool("buffer", true), buffer_);

    parameters.addParameter(param::factory::declareTrigger("clear"), std::bind(&MakeVideo::clear, this));

    parameters.addParameter(param::factory::declareTrigger("write"), std::bind(&MakeVideo::writeBuffer, this));
}
...
void MakeVideo::writeBuffer()
{
    ...
    cv::Size max_size(std::numeric_limits<int>::min(), std::numeric_limits<int>::min());
    ...
    for (CvMatMessage::ConstPtr& mat : msg_buffer_) {
        ...
        if (mat->value.type() != CV_8UC1 && mat->value.type() != CV_8UC3)
            throw std::runtime_error("Need 8UC1 or 8UC3!");
    }
