...
void ImageProviderDir::load(const std::string& directory)
{
    ...
    for (; dir != end; ++dir) {
        ...
        if (path.filename() == "img.ppm") {
            ...
            img_ = cv::imread(path.string(), cv::IMREAD_UNCHANGED);

        } else if (path.filename() == "mask.ppm") {
            std::cout << "mask" << std::endl;
            mask_ = cv::imread(path.string(), cv::IMREAD_GRAYSCALE);
        }
    }
...
void ImageProviderDir::next(cv::Mat& img, cv::Mat& mask)
{
    ...
    int d = 5;
    cv::rectangle(mask, cv::Rect(roi.x + d, roi.y + d, roi.width - 2 * d, roi.height - 2 * d), cv::Scalar::all(255), cv::FILLED);
}
