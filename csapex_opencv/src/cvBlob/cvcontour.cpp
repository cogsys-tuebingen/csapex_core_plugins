...
double cvContourPolygonCircularity(const CvContourPolygon* p)
{
    ...
    __CV_BEGIN__;
    {
        ...
        if (c >= 0.)
            return c;
        else  // This could happen if the blob it's only a pixel: the perimeter will
              // be 0. Another solution would be to force "cvContourPolygonPerimeter"
              // to be 1 or greater.
            return 0.;
    }
    __CV_END__;
...
void cvWriteContourPolygonSVG(const CvContourPolygon& p, const string& filename, const CvScalar& stroke, const CvScalar& fill)
{
    ...
    f << "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 20010904//EN\" "
         "\"http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd\">"
      << endl;
    f << "<svg xmlns=\"http://www.w3.org/2000/svg\" "
         "xmlns:xlink=\"http://www.w3.org/1999/xlink\" xml:space=\"preserve\" "
         "width=\""
      << maxx - minx << "px\" height=\"" << maxy - miny << "px\" viewBox=\"" << minx << " " << miny << " " << maxx << " " << maxy << "\" zoomAndPan=\"disable\" >" << endl;

    f << "<polygon fill=\"rgb(" << fill.val[0] << "," << fill.val[1] << "," << fill.val[2] << ")\" stroke=\"rgb(" << stroke.val[0] << "," << stroke.val[1] << "," << stroke.val[2]
      << ")\" stroke-width=\"1\" points=\"" << buffer.str() << "\"/>" << endl;
