...
struct CSAPEX_OPENCV_EXPORT Channel
{
    ...
    Channel(const Channel& copy) : name(copy.name), fp(copy.fp)
    {
        if (fp) {
            min_f = copy.min_f;
            max_f = copy.max_f;
...
public:
    ...
    Encoding(Iterator begin, Iterator end) : channels_(begin, end)
    {
    }
    Encoding(std::initializer_list<Channel> channels) : channels_(channels)
    {
    }
    Encoding(const std::vector<Channel>& channels) : channels_(channels)
    {
    }
