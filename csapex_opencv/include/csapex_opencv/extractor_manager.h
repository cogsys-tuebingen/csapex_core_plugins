...
class DetectorTraits
{
    ...
    static Small testKeypoint(__typeof__(&Class::keypoint));
    template <typename Class>
    static Big testKeypoint(...);

    template <typename Class>
    static Small testDescriptor(__typeof__(&Class::descriptor));
    template <typename Class>
    static Big testDescriptor(...);

public:
    enum
    {
        HasKeypoint = sizeof(testKeypoint<Any>(0)) == sizeof(Small)
    };
    enum
    {
        HasDescriptor = sizeof(testDescriptor<Any>(0)) == sizeof(Small)
    };
};
}  // namespace csapex
...
{
/**
 * @brief The ExtractorManager class manages all instances of feature detectors
 * and descriptor extractors. It functions like a proxy to the underlying
 * singleton classes.
 */
...
private:
    /**
     * @brief ExtractorManager functions like a proxy to the underlying singleton
     * classes.
     */
    ExtractorManager();
...
public:
    ...
    /**
     * @brief registerKeypointConstructor installs a new keypoint detector
     * instance
    ...
    /**
     * @brief registerDescriptorConstructor installs a new descriptor extractor
     * instance
     * @param key name of the instance, must be unique
...
protected:
    ...
    {
        ExtractorInitializerImp(KeypointInitializer kc, DescriptorInitializer dc, bool complete) : kc_(kc), dc_(dc), complete_(complete)
        {
        }
