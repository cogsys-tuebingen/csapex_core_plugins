...
VectorOptimizer::VectorOptimizer() : is_running_(false), has_new_parameter_(false), has_residual_(false), fitness_calls_(0), residual_(std::numeric_limits<double>::infinity())
{
}

void VectorOptimizer::setup(csapex::NodeModifier& modifier)
{
    in_ = modifier.addSlot<GenericValueMessage<double>>("residual", [this](const TokenPtr& residual) { residualCb(residual); });

    in_grad_ = modifier.addSlot<GenericVectorMessage>("gradient", [this](const TokenPtr& grad) { gradCb(grad); });

    in_start_ = modifier.addSlot<GenericVectorMessage>("start params", [this](const TokenPtr& msg) { startParamCb(msg); });
...
{
    params.addParameter(param::factory::declareTrigger("restart"), [this](param::Parameter*) { start(); });

    params.addParameter(param::factory::declareTrigger("stop"), [this](param::Parameter*) { stop(); });

    params.addParameter(param::factory::declareBool("trigger_immediately", false), trigger_immediately_);

    params.addParameter(param::factory::declareRange("polling_time", param::ParameterDescription("Time in ms after which is checked if data has arrived."), 1, 1000, 20, 1), wait_time_ms_);
