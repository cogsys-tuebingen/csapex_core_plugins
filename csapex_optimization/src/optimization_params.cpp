...
OptimizationParams::OptimizationParams() : set_bounds_(true), relative_bounds_(false), one_bound_for_all_(false), set_start_(false), problem_dim_(1)
{
}
...
void OptimizationParams::setupParameters(csapex::Parameterizable& params)
{
    ...
    params.addParameter(param::factory::declareRange("problem_dimension", 1, 100, 1, 1), [this](param::Parameter* p) {
    ...
    params.addParameter(param::factory::declareBool("set_bounds", false), [this](param::Parameter* p) {
    ...
    params.addConditionalParameter(param::factory::declareBool("bounds/relative", false), set_bounds_, relative_bounds_);

    params.addConditionalParameter(param::factory::declareBool("bounds/one_bound_for_all_params", false), set_bounds_, [this](param::Parameter* p) {
    ...
    params.addParameter(param::factory::declareBool("set_start", false), [this](param::Parameter* p) {
...
void OptimizationParams::boundParams()
{
    if (set_bounds_) {
        ...
        for (std::size_t i = 0; i < max_dim; ++i) {
            param::Parameter::Ptr intervall = param::factory::declareInterval("~bounds/interval_" + std::to_string(i), -10.0, 10.0, -10.0, 10.0, 0.01);
            //            std::function<void(param::Parameter* p)> cb = [this,
            //            i](param::Parameter* p){
            //                if(p->is<std::pair<double,double>>()){
            //                    p->as
            //                    std::pair<double, double> bound =
            //                    p->as<std::pair<double,double>>(); std::cout
            //                    <<"intervall: "<< i << " " << bound.first << " " <<
            //                    bound.second << std::endl; lower_bounds_[i] =
            //                    bound.first; upper_bounds_[i] = bound.second;
...
void OptimizationParams::startParams()
{
    if (set_start_) {
        for (std::size_t i = 0; i < problem_dim_; ++i) {
            param::Parameter::Ptr startVal = param::factory::declareValue("~start/x0_" + std::to_string(i), 0.1);
...
void OptimizationParams::removeBounds(std::size_t new_size)
{
    for (int i = ((int)problem_dim_) - 1; i >= (int)new_size; --i) {
        ...
        if (relative_bounds_) {
            intervall = param::factory::declareInterval("~bounds/interval_" + std::to_string(i), -10.0, 10.0, -0.1, 0.1, 0.1);
        } else {
            intervall = param::factory::declareInterval("~bounds/interval_" + std::to_string(i), -100.0, 100.0, -100.0, 100.0, 1.0);
        }
...
{
    for (int i = ((int)problem_dim_) - 1; i >= (int)new_size; --i) {
        param::Parameter::Ptr val = param::factory::declareValue("~start/x0_" + std::to_string(i), 0.1);
