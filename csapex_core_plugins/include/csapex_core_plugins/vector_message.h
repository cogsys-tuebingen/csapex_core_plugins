...
struct VectorMessage : public Message
{
    ...
    static VectorMessage::Ptr make(Token::Ptr type)
    {
...
private:
    VectorMessage(Token::Ptr type, const std::string& frame_id, Stamp stamp_micro_seconds);
...
public:
    ...
    template <typename T>
    static GenericVectorMessage::Ptr make(typename std::enable_if<std::is_base_of<Token, T>::value >::type* dummy = 0)
    {
        registerType<T>();
