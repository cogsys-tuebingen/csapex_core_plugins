...
TextDisplay::TextDisplay() : input_(nullptr)
{
}
...
void TextDisplay::setup(NodeModifier& node_modifier)
{
    ...
    slot_ = node_modifier.addSlot<connection_types::AnyMessage>("Display", [this](const TokenConstPtr& token) { display(token->getTokenData()); });
}
