...
public:
    ...
    {
        params.addParameter(param::ParameterFactory::declareRange("std_x",0.0,1.0,0.01,0.01),
                            [this](param::Parameter* p){
            double std = p->as<double>();
            noise_[0] = std::normal_distribution<double>(0, std);
        });

        params.addParameter(param::ParameterFactory::declareRange("std_y",0.0,1.0,0.01,0.01),
                            [this](param::Parameter* p){
            double std = p->as<double>();
            noise_[1] = std::normal_distribution<double>(0, std);
        });
        params.addParameter(param::ParameterFactory::declareRange("std_z",0.0,1.0,0.01,0.01),
                            [this](param::Parameter* p){
            double std = p->as<double>();
            noise_[2] = std::normal_distribution<double>(0, std);
        });
        params.addParameter(param::ParameterFactory::declareRange("std_roll",0.0,1.0,0.01,0.01),
                            [this](param::Parameter* p){
            double std = p->as<double>();
            noise_[3] = std::normal_distribution<double>(0, std);
        });
        params.addParameter(param::ParameterFactory::declareRange("std_pitch",0.0,1.0,0.01,0.01),
                            [this](param::Parameter* p){
            double std = p->as<double>();
            noise_[4] = std::normal_distribution<double>(0, std);
        });
        params.addParameter(param::ParameterFactory::declareRange("std_yaw",0.0,1.0,0.01,0.01),
                            [this](param::Parameter* p){
            double std = p->as<double>();
