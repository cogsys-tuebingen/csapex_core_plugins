...
RenderHistogram::RenderHistogram() : height_(480), width_(640)
{
}
...
void RenderHistogram::process()
{
    ...
    CvMatMessage::Ptr out(new CvMatMessage(enc::bgr, in->frame_id, in->stamp_micro_seconds));
    ...
    if (maxima.get() == nullptr) {
        int color_count = 0;
        for (std::vector<cv::Mat>::const_iterator it = in->value.histograms.begin(); it != in->value.histograms.end(); ++it, ++color_count) {
            int type = it->type();
            switch (type) {
                case CV_32FC1:
                    cslibs_vision::histogram::render_curve<float>(*it, cslibs_vision::histogram::COLOR_PALETTE.at(color_count % cslibs_vision::histogram::COLOR_PALETTE.size()), line_width,
                                                                  out->value);
                    break;
                case CV_32SC1:
                    cslibs_vision::histogram::render_curve<int>(*it, cslibs_vision::histogram::COLOR_PALETTE.at(color_count % cslibs_vision::histogram::COLOR_PALETTE.size()), line_width, out->value);
                    break;
                default:
        ...
        for (unsigned int i = 0; i < histograms.size(); ++i) {
            ...
            switch (type) {
                case CV_32F:
                    cslibs_vision::histogram::render_curve<float>(histograms.at(i), maxima->value.maxima.at(i),
                                                                  cslibs_vision::histogram::COLOR_PALETTE.at(i % cslibs_vision::histogram::COLOR_PALETTE.size()), line_width, 5, out->value);
                    break;
                case CV_32S:
                    cslibs_vision::histogram::render_curve<int>(histograms.at(i), maxima->value.maxima.at(i),
                                                                cslibs_vision::histogram::COLOR_PALETTE.at(i % cslibs_vision::histogram::COLOR_PALETTE.size()), line_width, 5, out->value);
                    break;
                default:
                    throw std::runtime_error("Only 32bit float or 32bit integer histograms supported!");
...
{
    parameters.addParameter(csapex::param::factory::declareRange("width", 200, 1000, width_, 10), std::bind(&RenderHistogram::update, this));
    parameters.addParameter(csapex::param::factory::declareRange("height", 200, 1000, height_, 10), std::bind(&RenderHistogram::update, this));
