...
void HistogramMaxima::process()
{
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        switch (type) {
            case CV_32F:
                cslibs_vision::histogram::find_maxima1D<float>(src, k, thresh, output->value.maxima.at(i));
                break;
            case CV_32S:
                cslibs_vision::histogram::find_maxima1D<int>(src, k, thresh, output->value.maxima.at(i));
                break;
            default:
