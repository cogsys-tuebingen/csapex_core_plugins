...
Histogram::Histogram()
  ...
  , append_(false)
  , min_max_value_(std::make_pair<float, float>(std::numeric_limits<float>::max(), std::numeric_limits<float>::min()))
{
}
...
void Histogram::process()
{
    ...
    cslibs_vision::histogram::histogram(in->value, histograms, mask, bins, ranges, uniform_, accumulate_);
...
{
    parameters.addParameter(csapex::param::factory::declareRange("bins", 2, 512, 255, 1), std::bind(&Histogram::update, this));
    parameters.addParameter(csapex::param::factory::declareBool("uniform", uniform_), std::bind(&Histogram::update, this));
    parameters.addParameter(csapex::param::factory::declareBool("accumulate", accumulate_), std::bind(&Histogram::update, this));
    parameters.addParameter(csapex::param::factory::declareBool("min max range", false), std::bind(&Histogram::update, this));
    parameters.addParameter(csapex::param::factory::declareBool("global min max", false), std::bind(&Histogram::update, this));
    parameters.addParameter(csapex::param::factory::declareBool("append histograms", false), std::bind(&Histogram::update, this));
}
