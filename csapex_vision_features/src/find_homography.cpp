...
FindHomography::FindHomography() : in_key_1(nullptr), in_key_2(nullptr)
{
}
...
void FindHomography::process()
{
    ...
    for (std::vector<cv::DMatch>::const_iterator it = matches->value.begin(); it != matches->value.end(); it++) {
    ...
    cv::drawMatches(image1->value, keypoints1->value, image2->value, keypoints2->value, matches->value, out->value, cv::Scalar::all(-1), cv::Scalar::all(-1), std::vector<char>(),
