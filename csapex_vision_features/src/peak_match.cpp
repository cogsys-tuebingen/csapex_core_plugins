...
CSAPEX_REGISTER_CLASS(csapex::PeakMatch, csapex::Node)
    ...
    : in_img_1(NULL)
{
    ...
    addParameter(param::ParameterFactory::declareRange("cluster_count", 1, 32, 1, 1));
    addParameter(param::ParameterFactory::declareRange("scaling", 1, 8, 1, 1));
    addParameter(param::ParameterFactory::declareRange("octaves", 1, 12, 1, 1));

    addParameter(param::ParameterFactory::declareRange("min_cluster_size", 1, 256, 1, 1));
}


void PeakMatch::process()
{
    ...
    if(des1->value.cols > 0 && des2->value.cols > 0) {
        ...
        if(false) {
            h = new HoughPeak<true, true> (param<int>("k"), param<int>("scaling"), param<int>("octaves"), m1, m2);
        } else {
            h = new HoughPeak<true, false>(param<int>("k"), param<int>("scaling"), param<int>("octaves"), m1, m2);
        }


        h->min_count = param<int>("min_cluster_size");
