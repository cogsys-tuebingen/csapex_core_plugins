...
namespace csapex {
namespace param {
class CSAPEX_PARAM_EXPORT AngleParameter
    : public ParameterImplementation<AngleParameter> {
public:
  typedef std::shared_ptr<AngleParameter> Ptr;

public:
  AngleParameter();
  explicit AngleParameter(const std::string &name,
                          const ParameterDescription &description, double angle,
                          double min = -M_PI, double max = M_PI);
  ...
  virtual void serialize(SerializationBuffer &data,
                         SemanticVersion &version) const override;
  virtual void deserialize(const SerializationBuffer &data,
                           const SemanticVersion &version) override;
...
template <> inline std::string serializationName<AngleParameter>() {
  return "angle";
}
