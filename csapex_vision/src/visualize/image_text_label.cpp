...
{
    std::map<std::string, int> positions = { { "BOTTOM_LEFT", BOTTOM_LEFT }, { "BOTTOM_RIGHT", BOTTOM_RIGHT }, { "TOP_LEFT", TOP_LEFT }, { "TOP_RIGHT", TOP_RIGHT } };
...
void ImageTextLabel::process()
{
    ...
    int basel_line = 0;
    cv::Size text_size = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, font_scale, thickness, &basel_line);
