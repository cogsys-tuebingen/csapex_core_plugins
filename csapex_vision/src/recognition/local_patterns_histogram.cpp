...
namespace
{
inline void lbp(const cv::Mat& src, std::vector<float>& result)
{
...
inline void ltp(const cv::Mat& src, const double k, std::vector<float>& result)
{
    std::vector<int> histogram;
    switch (src.type()) {
        case CV_8UC1:
...
void LocalPatternsHistogram::process()
{
    ...
    if (!msg::hasMessage(in_rois_)) {
        ...
        for (std::vector<RoiMessage>::const_iterator it = in_rois->begin(); it != in_rois->end(); ++it) {
            FeaturesMessage feature_msg;
            feature_msg.type = FeaturesMessage::Type::CLASSIFICATION;
...
{
    std::map<std::string, int> types = { { "LBP", LBP }, { "LTP", LTP } };

    csapex::param::Parameter::Ptr type = csapex::param::factory::declareParameterSet("pattern", types, (int)LBP);
    std::function<bool()> condition = [type]() { return type->as<int>() == LTP; };

    parameters.addParameter(type);
    parameters.addConditionalParameter(csapex::param::factory::declareRange("k1", -100.0, 100.0, 0.0, 0.1), condition);
}
