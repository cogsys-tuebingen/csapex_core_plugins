...
void LocalPatterns::process()
{
    ...
    if (b) {
        ...
        switch (t) {
            case LBP:
                cv::copyMakeBorder(in->value, working, size, size, size, size, cv::BORDER_REFLECT_101);
...
void LocalPatterns::setupParameters(Parameterizable& parameters)
{
    std::map<std::string, int> types = { { "LBP", LBP },
                                         ...
                                         { "WLD", WLD },
                                         { "WLD_SHORT", WLD_SHORT },
                                         { "WLD_ORIENTED", WLD_ORIENTED },
                                         { "HOMOGENITY", HOMOGENITY },
                                         { "HOMOGENITY_TEX", HOMOGENITY_TEX } };

    csapex::param::Parameter::Ptr type = csapex::param::factory::declareParameterSet("descriptor", types, (int)LBP);

    std::function<bool()> condition_threshold = [type]() {
        Type t = (Type)type->as<int>();
        return t != LBP_VAR && t != WLD && t != WLD_ORIENTED && t != WLD_SHORT && t != HOMOGENITY && t != HOMOGENITY_TEX;
    };

    std::function<bool()> condition_neighbours = [type]() {
        Type t = (Type)type->as<int>();
        return t == LBP_EXT || t == LBP_VAR || t == LTP_EXT;
    };

    parameters.addParameter(type);

    parameters.addConditionalParameter(csapex::param::factory::declareRange("k", csapex::param::ParameterDescription("Center difference offset."), -100.0, 100.0, 0.0, 0.1), condition_threshold);

    parameters.addConditionalParameter(csapex::param::factory::declareRange("neighbours", csapex::param::ParameterDescription("Amount of neighbours."), 2, 31, 8, 1), condition_neighbours);

    parameters.addConditionalParameter(csapex::param::factory::declareRange("radius", csapex::param::ParameterDescription("Radius of neighbourhood."), 1, 16, 8, 1), condition_neighbours);

    parameters.addParameter(csapex::param::factory::declareBool("add border", csapex::param::ParameterDescription("Add boarder to adjust pattern analysis size to image size."), false));
}
