...
HOGDetector::HOGDetector() : prev_svm_type_(NONE), svm_type_(NONE)
{
}

void HOGDetector::setupParameters(Parameterizable& parameters)
{
    std::map<std::string, int> svm_types = { { "default", DEFAULT }, { "custom", CUSTOM }, { "daimler", DAIMLER } };
    param::Parameter::Ptr svm_param = param::factory::declareParameterSet("svm/type", svm_types, (int)DEFAULT);
    parameters.addParameter(svm_param, svm_type_);

    parameters.addParameter(csapex::param::factory::declareRange("svm/thresh", -10000.0, 10000.0, 0.0, 0.1), svm_thresh_);

    std::function<bool()> custom_active = [svm_param]() { return svm_param->as<int>() == CUSTOM; };
    parameters.addConditionalParameter(param::factory::declareFileInputPath("svm/path", "", "*.yml *.yaml *.tar.gz *.yaml.gz"), custom_active, std::bind(&HOGDetector::load, this));
    parameters.addConditionalParameter(param::factory::declareBool("svm/load_threshold", true), custom_active, std::bind(&HOGDetector::load, this));

    /// scan mode
    std::map<std::string, int> scan_modes = { { "single scale", SINGLE_SCALE }, { "multi scale", MULTI_SCALE } };
    parameters.addParameter(param::factory::declareParameterSet("hog/scan_mode", scan_modes, (int)SINGLE_SCALE), scan_mode_);

    parameters.addParameter(param::factory::declareRange("hog/levels", param::ParameterDescription("Scale levels to observe."), 1, 128, 64, 1), hog_.nlevels);
    ...
    parameters.addConditionalParameter(param::factory::declareBool("hog/signed_gradient", param::ParameterDescription("Un-/directed gradients."), hog_.signedGradient), custom_active,
                                       signed_gradient_);

    parameters.addConditionalParameter(param::factory::declareRange("hog/gradient_bins", param::ParameterDescription("Amount of gradient bins."), 2, 18, hog_.nbins, 1), custom_active, n_bins_);

    parameters.addConditionalParameter(param::factory::declareRange("hog/cells_x", param::ParameterDescription("Cells in x direction."), 2, 16, 8, 1), custom_active, cells_x_);

    parameters.addConditionalParameter(param::factory::declareRange("hog/cells_y", param::ParameterDescription("Cells in x direction."), 2, 16, 16, 1), custom_active, cells_y_);

    parameters.addConditionalParameter(param::factory::declareRange("hog/cell_size", param::ParameterDescription("Size of the cells."), 4, 16, 8, 1), custom_active, cell_size_);
    parameters.addConditionalParameter(param::factory::declareRange("hog/block_size", param::ParameterDescription("Cell count in both dimension of a block."), 1, 4, 2, 1), custom_active, block_size_);
    parameters.addConditionalParameter(param::factory::declareRange("hog/bock_stride", param::ParameterDescription("Overlap of each block in cells."), 1, 3, 1, 1), custom_active, block_stride_);
}
...
void HOGDetector::setParameters(const int cell_size, const int cells_x, const int cells_y, const int block_size, const int block_stride, const int bins, const bool signed_gradient)
{
    ...
    if (hog_.blockSize.width != block_size_px || hog_.blockSize.height != block_size_px) {
    ...
    if (hog_.blockStride.width != block_stride_px || hog_.blockStride.height != block_stride_px) {
