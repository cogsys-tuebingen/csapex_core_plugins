...
HOGClassifier::HOGClassifier() : prev_svm_type_(NONE)
{
}
...
void HOGClassifier::setupParameters(Parameterizable& parameters)
{
    ...
    parameters.addParameter(param::factory::declareParameterSet("svm/thershold_type", svm_thresh_types, (int)GREATER), svm_thresh_type_);

    std::function<bool()> custom_active = [svm_param]() { return svm_param->as<int>() == CUSTOM; };
    parameters.addConditionalParameter(param::factory::declareFileInputPath("svm/path", "", "*.yml *.yaml *.tar.gz *.yaml.gz"), custom_active, std::bind(&HOGClassifier::load, this));

    parameters.addParameter(param::factory::declareRange("svm/thresh", -10000.0, 10000.0, 0.0, 0.01), svm_thresh_);

    parameters.addParameter(param::factory::declareBool("hog/mirror", param::ParameterDescription("Mirror for classification."), false), mirror_);
...
void HOGClassifier::setParameters(const int cell_size, const int cells_x, const int cells_y, const int block_size, const int block_stride, const int bins, const bool signed_gradient)
{
