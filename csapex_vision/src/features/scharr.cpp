...
void Scharr::setupParameters(Parameterizable& parameters)
{
    ...
    std::map<std::string, int> types = { { "DX1", DX1 }, { "DY1", DY1 }, { "Strength", STRENGTH } };

    parameters.addParameter(csapex::param::factory::declareParameterSet("derive", types, (int)DX1), std::bind(&Scharr::update, this));
}
