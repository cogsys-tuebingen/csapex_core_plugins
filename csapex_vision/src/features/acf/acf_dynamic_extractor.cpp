...
ACFDynamicExtractor::ACFDynamicExtractor() : ratio_w_h_(0.0)
{
}
...
{
    // window setting
    parameters.addParameter(param::factory::declareRange("window/width", 10, 1024, 64, 1), std::bind(&ACFDynamicExtractor::updateWindow, this));
    parameters.addParameter(param::factory::declareRange("window/height", 10, 1024, 128, 1), std::bind(&ACFDynamicExtractor::updateWindow, this));
    parameters.addParameter(param::factory::declareBool("window/keep_ratio", false), keep_ratio_);
    parameters.addParameter(param::factory::declareBool("window/mirror", false), mirror_);

    // kernel
    static const std::map<std::string, int> kernel_types = { { "1D", cslibs_vision::ACF::Parameters::KERNEL_1D },
                                                             { "2D", cslibs_vision::ACF::Parameters::KERNEL_2D },
                                                             { "NONE", cslibs_vision::ACF::Parameters::NONE } };
    parameters.addParameter(param::factory::declareParameterSet("kernel_type", kernel_types, static_cast<int>(cslibs_vision::ACF::Parameters::KERNEL_2D)),
    ...
    static const int channel_default = cslibs_vision::ACFDynamic::Parameters::MAGNITUDE | cslibs_vision::ACFDynamic::Parameters::HOG | cslibs_vision::ACFDynamic::Parameters::LUV;

    parameters.addParameter(param::factory::declareParameterBitSet("channels", channel_types, channel_default), acf_params_.channel_types);

    // HOG channel parameter
    parameters.addParameter(param::factory::declareRange("hog/bin_size", 5.0, 90.0, 30.0, 0.1), acf_params_.hog_bin_size);
    parameters.addParameter(param::factory::declareBool("hog/directed", false), acf_params_.hog_directed);

    // HOG/magnitude channel parameter
    parameters.addParameter(param::factory::declareBool("magnitude/normalize", true), acf_params_.normalize_magnitude);

    // LUV channel parameter
    parameters.addParameter(param::factory::declareBool("luv/normalize", true), acf_params_.normalize_luv);

    // LBP/LTP/WLD/HOMOGENITY channel parameter
    parameters.addParameter(param::factory::declareRange("pattern/k", -1.0, 1.0, 0.0, 0.01), acf_params_.k);
    parameters.addParameter(param::factory::declareBool("pattern/normalize", false), acf_params_.normalize_patterns);
}
...
void ACFDynamicExtractor::process()
{
    ...
    if (in_img->getEncoding().matches(enc::bgr) && in_img->getEncoding().matches(enc::mono)) {
