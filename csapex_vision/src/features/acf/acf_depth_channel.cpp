...
{
    parameters.addParameter(param::factory::declareRange("window/width", 10, 1024, 64, 1), std::bind(&ACFDepthChannel::updateWindow, this));
    parameters.addParameter(param::factory::declareRange("window/height", 10, 1024, 128, 1), std::bind(&ACFDepthChannel::updateWindow, this));
    ...
    parameters.addParameter(param::factory::declareRange("aggregate/block_size", 1, 32, 4, 1), block_size_);
    ...
    parameters.addParameter(param::factory::declareParameterSet("channel/type", available_types, static_cast<int>(Type::BINARY)), reinterpret_cast<int&>(type_));
    ...
    parameters.addParameter(param::factory::declareParameterSet("channel/method", available_methods, static_cast<int>(Method::MEDIAN)), reinterpret_cast<int&>(method_));

    parameters.addParameter(param::factory::declareRange("channel/threshold", 0.0, 1000.0, 0.1, 0.01), threshold_);

    parameters.addParameter(param::factory::declareBool("channel/normalize", false), normalize_);
}
...
std::vector<float> ACFDepthChannel::extractChannel(const cv::Mat& depth_map) const
{
    ...
    switch (type_) {
        case Type::TERNARY:
            std::transform(aggregated_depth_map.begin<float>(), aggregated_depth_map.end<float>(), std::back_inserter(feature), [&](float value) {
                if (normalize_) {
