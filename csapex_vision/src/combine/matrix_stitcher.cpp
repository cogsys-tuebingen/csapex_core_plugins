...
namespace
{
void stitch(const cv::Mat& src1, const cv::Mat& src2, const bool vertical, const unsigned int offset, cv::Mat& dst)
{
    ...
    if (vertical) {
        pad_rows = offset;
        dst = cv::Mat(rows * 2 + offset, cols, src1.type(), cv::Scalar::all(0));
    } else {
        pad_cols = offset;
        dst = cv::Mat(rows, cols * 2 + offset, src1.type(), cv::Scalar::all(0));
    }
    ...
    roi_rect = cv::Rect((vertical ? 0 : cols) + x_off, (vertical ? rows : 0) + y_off, src2.cols, src2.rows);
...
{
    std::map<std::string, int> modes = { { "HORIZONTAL", HORIZONTAL }, { "VERTICAL", VERTICAL } };
