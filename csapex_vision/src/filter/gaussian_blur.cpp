...
GaussianBlur::GaussianBlur() : kernel_(1), sigma_x_(0.1), sigma_y_(0.0)
{
}

void GaussianBlur::setupParameters(Parameterizable& parameters)
{
    parameters.addParameter(csapex::param::factory::declareRange("kernel", 1, 255, kernel_, 2), std::bind(&GaussianBlur::update, this));
    parameters.addParameter(csapex::param::factory::declareRange("sigma x", 0.1, 128.0, sigma_x_, 0.1), std::bind(&GaussianBlur::update, this));
    parameters.addParameter(csapex::param::factory::declareRange("sigma y", 0.0, 128.0, sigma_y_, 0.1), std::bind(&GaussianBlur::update, this));
}
