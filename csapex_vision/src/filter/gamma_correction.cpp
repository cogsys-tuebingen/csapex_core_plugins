...
{
    std::map<std::string, int> types = { { "power law", (int)POWER_LAW }, { "logarithm", (int)LOGARITHM } };

    csapex::param::Parameter::Ptr type = csapex::param::factory::declareParameterSet<int>("type", csapex::param::ParameterDescription("The type of transformation to apply"), types, POWER_LAW);
    parameters.addParameter(type);

    addParameter(csapex::param::factory::declareRange("c", csapex::param::ParameterDescription("Constant factor in  dst = c * log(src + 1)"), 0.1, 255.0, 1.0, 0.01));

    addConditionalParameter(csapex::param::factory::declareRange("gamma", csapex::param::ParameterDescription("Constant factor in  dst = c * (src ^ gamma)"), 0.001, 10.0, 1.0, 0.001),
