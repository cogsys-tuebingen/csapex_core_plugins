...
namespace
{
inline void normalize(const cv::Mat& src, const std::vector<double>& mean, const std::vector<double>& dev, cv::Mat& dst)
{
...
inline void normalizeC(const cv::Mat& src, const std::vector<double>& mean, const std::vector<double>& dev, cv::Mat& dst)
{
    std::vector<cv::Mat> channels;
    cv::split(src, channels);
    for (unsigned int i = 0; i < channels.size(); ++i) {
...
void MeanStdDevNormalization::process()
{
    ...
    if (depth == CV_64F || depth == CV_32F)
        normalize(in_mat->value, *in_mean, *in_dev, out->value);
    else
        normalizeC(in_mat->value, *in_mean, *in_dev, out->value);
