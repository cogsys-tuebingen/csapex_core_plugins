...
void csapex::CameraCalibration::process()
{
    ...
    if(found) {
        ...
        case CIRCLES_GRID:
            for(int i = 0 ; i < pattern_size_.height ; ++i) {
                for(int j = 0 ; j < pattern_size_.width ; ++j) {
                    object_points.emplace_back(cv::Point3f(pattern_scale_ * j,
                                                           pattern_scale_ * i,
                                                           0.f));
                }
            }
        ...
        case ASYMMETRIC_CIRCLES_GRID:
            for(int i = 0 ; i < pattern_size_.height ; ++i) {
                for(int j = 0 ; j < pattern_size_.width ; ++j) {
                    object_points.emplace_back(cv::Point3f(pattern_scale_ * (2*j + i % 2),
                                                           pattern_scale_ * i ,
                                                           0.f));
                }
            }
...
void csapex::CameraCalibration::setupParameters(Parameterizable& parameters)
{
    ...
    std::map<std::string, int> types {
        {"chessboard", static_cast<int>(CHESSBOARD)},
