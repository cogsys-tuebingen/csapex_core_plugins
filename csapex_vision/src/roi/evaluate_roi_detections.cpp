...
EvaluateROIDetections::EvaluateROIDetections() : human_parts_found_({ 0, 0 }), frame_count_(0)
{
}
...
{
    std::map<std::string, int> mode_types = { { "IGNORE_PARTLY_VISIBLE", IGNORE_PARTLY_VISIBLE }, { "INTEGRATE_PARTLY_VISIBLE", INTEGRATE_PARTLY_VISIBLE } };
    std::map<std::string, int> overlap_mode_types = {
        { "MAX", OVERLAP_MAX },
        { "IOU", OVERLAP_IOU },
    };

    parameters.addParameter(param::factory::declareParameterSet("mode", mode_types, (int)IGNORE_PARTLY_VISIBLE), (int&)mode_);
    parameters.addParameter(param::factory::declareFileOutputPath("statistic path", "", "*.txt"), path_of_statistic_);
    parameters.addParameter(param::factory::declareParameterSet("overlap_mode", overlap_mode_types, static_cast<int>(OVERLAP_MAX)), reinterpret_cast<int&>(overlap_mode_));
    parameters.addParameter(param::factory::declareRange("overlap", 0.25, 1.0, 0.7, 0.01), percentage_of_overlap_);

    parameters.addParameter(param::factory::declareTrigger("save"), std::bind(&EvaluateROIDetections::save, this));
    parameters.addParameter(param::factory::declareTrigger("reset"), std::bind(&EvaluateROIDetections::resetConfusion, this));
}
...
inline double overlap_max(const cv::Rect& prediction, const cv::Rect& groundtruth)
{
    return (prediction & groundtruth).area() / (double)std::max(prediction.area(), groundtruth.area());
}
inline double overlap_iou(const cv::Rect& prediction, const cv::Rect& groundtruth)
{
    return (prediction & groundtruth).area() / (double)(prediction | groundtruth).area();
}

inline void evaluateIgnoringPartlyVisible(const RoiMessagesConstPtr& groundtruth, const RoiMessagesConstPtr& predition, const double min_overlap, ConfusionMatrix& confury,
                                          std::array<std::size_t, 2>& human_parts, std::function<double(const cv::Rect&, const cv::Rect&)> overlap_fn, DebugRois& debug)
{
...
inline void evaluteIntegratingPartlyVisible(const RoiMessagesConstPtr& groundtruth, const RoiMessagesConstPtr& prediction, const double min_overlap, ConfusionMatrix& confury,
                                            std::function<double(const cv::Rect&, const cv::Rect&)> overlap_fn, DebugRois& debug)
{
    ...
    for (std::size_t i = 0; i < size_groundtruth; ++i) {
        ...
        if (max_overlap > min_overlap) {
            ...
            if (roi_gt.value.classification() == EvaluateROIDetections::HUMAN) {
                ...
                /// we can find a match but it has not to be valid, if so good,
...
void EvaluateROIDetections::process()
{
    ...
    RoiMessagesConstPtr in_prediction = msg::getMessage<GenericVectorMessage, RoiMessage>(in_prediction_);
    RoiMessagesConstPtr in_groundtruth = msg::getMessage<GenericVectorMessage, RoiMessage>(in_groundtruth_);

    impl::DebugRois out_rois;

    /// every recieved message equals to one processed frame/image, so just
    /// increment the count
    frame_count_ += 1;
    ...
    switch (mode_) {
        case IGNORE_PARTLY_VISIBLE:
            impl::evaluateIgnoringPartlyVisible(in_groundtruth, in_prediction, percentage_of_overlap_, confusion_, human_parts_found_, overlap_fn, out_rois);
            break;
        case INTEGRATE_PARTLY_VISIBLE:
            impl::evaluteIntegratingPartlyVisible(in_groundtruth, in_prediction, percentage_of_overlap_, confusion_, overlap_fn, out_rois);
            break;
        default:
...
void EvaluateROIDetections::save() const
{
    ...
    for (const auto& e : confusion_.histogram) {
        out << e.first.first << " : " << e.first.second << " " << e.second << std::endl;
    }
