...
public:
    ...
    {
        parameters.addParameter(param::factory::declareValue<std::string>("classes", param::ParameterDescription("Comma separated list of allowed classification labels"), "0,1"),
    ...
    void updateLabels()
    {
        ...
        try {
            std::transform(labels.begin(), labels.end(), std::inserter(labels_, labels_.begin()), [](const std::string& text) { return boost::lexical_cast<int>(text); });
