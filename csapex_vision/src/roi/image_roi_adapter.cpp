...
ImageRoiAdapter::ImageRoiAdapter(NodeFacadeImplementationPtr worker, NodeBox* parent, std::weak_ptr<ImageRoi> node)
  : DefaultNodeAdapter(worker, parent), wrapped_(node), pixmap_(nullptr), view_(new QGraphicsView), empty(32, 32, QImage::Format_RGB16), painter(&empty), middle_button_down_(false), loaded_(false)
{
...
GenericStatePtr ImageRoiAdapter::getState() const
{
    return std::shared_ptr<State>(new State(state));
}

void ImageRoiAdapter::setParameterState(GenericStatePtr memento)
{
...
void ImageRoiAdapter::display(const QImage& img)
{
    ...
    bool change = state.last_size != img.size() || state.last_roi_size != roi_size;
...
void ImageRoiAdapter::submit()
{
    ...
    Roi roi(top_left.x(), top_left.y(), bottom_right.x() - top_left.x(), bottom_right.y() - top_left.y());
