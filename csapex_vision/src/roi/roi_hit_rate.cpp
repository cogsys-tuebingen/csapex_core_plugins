...
public:
    ...
    ROIHitRate() : ground_truth_rois_(0), ground_truth_rois_hit_(0)
    {
    }
    ...
    virtual void setupParameters(Parameterizable& parameters) override
    {
        ...
        parameters.addParameter(param::factory::declareRange("min_overlap", 0.0, 1.0, 0.7, 0.01), min_overlap_);
        parameters.addParameter(param::factory::declareFileOutputPath("statistic path", "", "*.txt"), path_of_statistic_);

        parameters.addParameter(param::factory::declareTrigger("reset"), std::bind(&ROIHitRate::reset, this));
        parameters.addParameter(param::factory::declareTrigger("save"), std::bind(&ROIHitRate::save, this));
    }
    virtual void process() override
    {
        RoiMessagesPtr input_gt = msg::getMessage<GenericVectorMessage, RoiMessage>(input_gt_);
        RoiMessagesPtr input_prediction = msg::getMessage<GenericVectorMessage, RoiMessage>(input_prediction_);
...
private:
    ...
    void save()
    {
        ...
        out << ground_truth_rois_ << " : " << ground_truth_rois_hit_ << " : " << (ground_truth_rois_hit_ / (double)ground_truth_rois_) << std::endl;
        out.close();
    }
