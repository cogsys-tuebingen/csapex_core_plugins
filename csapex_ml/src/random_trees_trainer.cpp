...
RandomTreesTrainer::RandomTreesTrainer() : MachineLearningNode("rforest.yaml"), categories_(0)
{
}
...
void RandomTreesTrainer::setupParameters(Parameterizable& parameters)
{
    ...
    addParameter(csapex::param::factory::declareRange<int>("classes", csapex::param::ParameterDescription("Number of classes to learn."), 0, 100, 2, 1),
                 std::bind(&RandomTreesTrainer::updatePriors, this));
    ;

    addParameter(csapex::param::factory::declareRange<int>("max depth",
                                                           csapex::param::ParameterDescription("the depth of the tree. A low value will likely underfit and "
                                                                                               "conversely a high value will likely overfit.\n"
                                                                                               "The optimal value can be obtained using cross validation or other "
                                                                                               "suitable methods."),
                                                           1, 64, 8, 1));
    ;
    addParameter(csapex::param::factory::declareRange<int>("min sample count",
                                                           csapex::param::ParameterDescription("minimum samples required at a leaf node for it to be split.\n"
                                                                                               "A reasonable value is a small percentage of the total data e.g. "
                                                                                               "1%."),
                                                           0, 64, 10, 1));
    addParameter(csapex::param::factory::declareRange<double>("regression accuracy",
                                                              csapex::param::ParameterDescription("Termination criteria for regression trees. \n"
                                                                                                  ...
                                                                                                  "and values of train samples in this node \n"
    ...
    auto surrogate_p = csapex::param::factory::declareBool("use surrogates",
                                                           csapex::param::ParameterDescription("If true then surrogate splits will be built. \n"
                                                                                               "These splits allow to work with missing data and "
                                                                                               "compute variable importance correctly."),
                                                           use_surrogates)
                           .build();
    ...
    addParameter(csapex::param::factory::declareRange<int>("max categories",
                                                           csapex::param::ParameterDescription("Cluster possible values of a categorical variable into K <= "
                                                                                               ...
                                                                                               "make a split, takes more than max_categories values,\n"
                                                                                               "the precise best subset estimation may take a very long time "
                                                                                               "because the algorithm is exponential.\n"
                                                                                               "Instead, many decision trees engines (including ML) try to find "
                                                                                               "sub-optimal split in this case by clustering all\n"
                                                                                               "the samples into max_categories clusters that is some categories "
                                                                                               "are merged together.\n"
                                                                                               "The clustering is applied only in n>2-class classification problems "
                                                                                               "for categorical variables\n"
                                                                                               "with N > max_categories possible values.\n"
                                                                                               "In case of regression and 2-class classification the optimal split "
                                                                                               "can be found efficiently without employing clustering,\n"
                                                                                               "thus the parameter is not used in these cases."),
                                                           0, 100, 15, 1));
    ;

    addParameter(csapex::param::factory::declareBool("calc_var_importance",
                                                     csapex::param::ParameterDescription("If true then variable importance will be calculated and then it can "
                                                                                         "be retrieved by CvRTrees::get_var_importance()."),
                                                     false));

    addParameter(csapex::param::factory::declareRange<int>("nactive_vars",
                                                           csapex::param::ParameterDescription("The size of the randomly selected subset of features at each tree "
                                                                                               "node and that are used to find the best split(s).\n"
                                                                                               "If you set it to 0 then the size will be set to the square root of "
                                                                                               "the total number of features."),
                                                           0, 100, 0, 1));
    addParameter(csapex::param::factory::declareRange<int>("max_num_of_trees_in_the_forest",
                                                           csapex::param::ParameterDescription("The maximum number of trees in the forest (surprise, surprise). "
                                                                                               "Typically the more trees you have the better the accuracy.\n"
                                                                                               "However, the improvement in accuracy generally diminishes and "
                                                                                               "asymptotes pass a certain number of trees.\n"
                                                                                               "Also to keep in mind, the number of tree increases the prediction "
                                                                                               "time linearly."),
                                                           1, 1024, 16, 1));
...
bool RandomTreesTrainer::processCollection(std::vector<connection_types::FeaturesMessage>& collection)
{
...
#if CV_MAJOR_VERSION == 2
    CvRTParams params(readParameter<int>("max depth"), readParameter<int>("min sample count"), readParameter<double>("regression accuracy"), readParameter<bool>("use surrogates"),
                      readParameter<int>("max categories"), priors_.data(), readParameter<bool>("calc_var_importance"), readParameter<int>("nactive_vars"),
                      readParameter<int>("max_num_of_trees_in_the_forest"), readParameter<double>("forest_accuracy"), readParameter<int>("termcrit_type"));

    cv::Mat var_type(train_data.cols + 1, 1, CV_8U, CV_VAR_NUMERICAL);

    cv::RandomTrees rtrees;
    ainfo << "Started training of tree with feature width " << collection.front().value.size() << " and " << collection_size << " samples!" << std::endl;
    ...
    rtrees->setUseSurrogates(false);  // not calling readParameter<bool>("use
                                      // surrogates") -> not implemented
