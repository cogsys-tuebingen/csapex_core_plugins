...
{
    parameters.addParameter(csapex::param::factory::declareFileInputPath("file", "mlp.yaml"), [this](param::Parameter* p) {
...
void MLPCv::process()
{
    std::shared_ptr<std::vector<FeaturesMessage> const> input = msg::getMessage<GenericVectorMessage, FeaturesMessage>(input_);
...
void MLPCv::classify(const FeaturesMessage& input, FeaturesMessage& output)
{
    output = input;
