...
GradientBoostedTreesTrainer::GradientBoostedTreesTrainer() : classes_(0)
{
}
...
void GradientBoostedTreesTrainer::setupParameters(Parameterizable& parameters)
{
    ...
    /// output path
    parameters.addParameter(param::factory::declareFileOutputPath("path", "", "*.yaml"), path_);

    /// ensemble specific parameters
    parameters.addParameter(param::factory::declareRange<int>("gb/weak_count", param::ParameterDescription("Count of trees in the ensemble."), 1, 4096, params_.weak_count, 1), params_.weak_count);
    std::map<std::string, int> loss_function_type = {
        { "ABSOLUTE_LOSS", CvGBTrees::ABSOLUTE_LOSS }, { "HUBER_LOSS", CvGBTrees::HUBER_LOSS }, { "DEVIANCE_LOSS", CvGBTrees::DEVIANCE_LOSS }, { "SQUARED_LOSS", CvGBTrees::SQUARED_LOSS }
    };
    parameters.addParameter(
        param::factory::declareParameterSet("gb/loss_function_type", param::ParameterDescription("Loss function used for ensemble training."), loss_function_type, (int)CvGBTrees::ABSOLUTE_LOSS));
    parameters.addParameter(param::factory::declareRange<double>("gb/subsample_portion",
                            ...
                            subsample_portion_);
    parameters.addParameter(param::factory::declareRange<double>("gb/shrinkage",
    ...
    parameters.addParameter(param::factory::declareRange<int>("dtree/classes", param::ParameterDescription("Number of classes to learn."), 2, 100, 2, 1),
                            std::bind(&GradientBoostedTreesTrainer::updatePriors, this));
    ;

    parameters.addParameter(param::factory::declareRange<int>("dtree/max_depth",
                            ...
                            params_.max_depth);
    ;
    parameters.addParameter(param::factory::declareRange<int>("dtree/min_sample_count",
                                                              param::ParameterDescription("If the number of samples in a node is less than this parameter "
                                                                                          "then the node will not be split."),
                                                              0, 128, params_.min_sample_count, 1),
                            params_.min_sample_count);
    parameters.addParameter(param::factory::declareRange<double>("dtree/regression_accuracy",
    ...
    ;
    parameters.addParameter(param::factory::declareBool("use surrogates",
    ...
    ;
    parameters.addParameter(param::factory::declareRange<int>("dtree/max_categories",
                                                              param::ParameterDescription("Cluster possible values of a categorical variable into K < "
    ...
    ;
    parameters.addParameter(param::factory::declareRange<int>("dtree/cv_folds",
                                                              param::ParameterDescription("If cv_folds > 1 then prune a tree with K-fold cross-validation "
                                                                                          "where K is equal to cv_folds."),
                                                              0, 100, params_.cv_folds, 1),
                            params_.cv_folds);
    ;
    parameters.addParameter(param::factory::declareBool("dtree/use_1se_rule",
                            ...
                            params_.use_1se_rule);
    ;
    parameters.addParameter(param::factory::declareBool("dtree/truncate_pruned_tree",
                                                        param::ParameterDescription("If true then pruned branches are physically removed from the "
                                                                                    "tree. \n"
