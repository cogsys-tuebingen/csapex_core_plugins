...
void ARFFFileExporter::setupParameters(Parameterizable& parameters)
{
    ...
    parameters.addParameter(param::ParameterFactory::
                            declareText("relation name",
...
bool ARFFFileExporter::processCollection(std::vector<connection_types::FeaturesMessage> &collection)
{
    ...
    for(std::size_t i = 0 ; i < step ; ++i) {
        arff.add_attr(std::make_shared<ArffAttribute>(ArffAttribute("feature_" + std::to_string(i),
                                                 cslibs_arff::NUMERIC)));
    }
