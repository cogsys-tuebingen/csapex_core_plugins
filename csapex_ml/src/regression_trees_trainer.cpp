...
void RegressionTreesTrainer::setupParameters(csapex::Parameterizable& params)
{
    ...
    params.addParameter(csapex::param::factory::declareRange<int>("max depth",
                                                                  ...
                                                                  1, 64, 8, 1));
    ;
    params.addParameter(csapex::param::factory::declareRange<int>("min sample count",
                                                                  csapex::param::ParameterDescription("minimum samples required at a leaf node for it to be split.\n"
                                                                                                      ...
                                                                                                      "1%."),
                                                                  0, 64, 10, 1));
    params.addParameter(csapex::param::factory::declareRange<double>("regression accuracy",
    ...
    params.addParameter(csapex::param::factory::declareBool("use surrogates",
                                                            ...
                                                            use_surrogates));
    params.addParameter(csapex::param::factory::declareRange<int>("max categories",
    ...
    params.addParameter(csapex::param::factory::declareBool("calc_var_importance",
    ...
    params.addParameter(csapex::param::factory::declareRange<int>("nactive_vars",
                                                                  ...
                                                                  0, 100, 0, 1));
    params.addParameter(csapex::param::factory::declareRange<int>("max_num_of_trees_in_the_forest",
...
#pragma message "Regression Trees are currently not implemented for OpenCV 2."
        ...
        std::cout << std::endl;
        cv::Ptr<cv::ml::TrainData> train_data_struct = cv::ml::TrainData::create(train_data, tflag, responses.at(i), cv::noArray(), cv::noArray(), cv::noArray(), var_type);

        ainfo << "Started training for tree # " << i << " with " << train_data.rows << " samples!" << std::endl;
        if (rtrees->train(train_data_struct)) {
            std::string label = prefix + std::to_string(i);
            fs << label << "{";
            rtrees->write(fs);
            fs << "}";
            ainfo << "Finished training for tree # " << i << "!" << std::endl;
        } else {
            return false;
        }
#endif
