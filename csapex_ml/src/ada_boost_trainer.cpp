...
void AdaBoostTrainer::setupParameters(Parameterizable& parameters)
{
    ...
    parameters.addParameter(param::factory::declareFileOutputPath("boost/path", "", "*.yaml, *.tar.gz"), path_);
...
#if CV_MAJOR_VERSION == 2
    std::map<std::string, int> split_criteria = { { "DEFAULT", Boost::DEFAULT }, { "GINI", Boost::GINI }, { "MISCLASS", Boost::MISCLASS }, { "SQERR", Boost::SQERR } };

    parameters.addParameter(param::factory::declareParameterSet("boost/split_criteria", split_criteria, (int)Boost::DEFAULT), split_criteria_);
#endif

    std::map<std::string, int> boost_types = { { "DISCRETE", Boost::DISCRETE }, { "REAL", Boost::REAL }, { "LOGIT", Boost::LOGIT }, { "GENTLE", Boost::GENTLE } };

    parameters.addParameter(param::factory::declareParameterSet("boost/type", boost_types, (int)Boost::DISCRETE), boost_type_);

    parameters.addParameter(param::factory::declareRange("boost/classifier_count", 1, 4096, 100, 1), weak_count_);

    parameters.addParameter(param::factory::declareRange("boost/trim_rate", 0.0, 1.0, 0.0, 0.01), weight_trim_rate_);

    parameters.addParameter(param::factory::declareRange("boost/max_depth", 1, 64, 8, 1), max_depth_);

    parameters.addParameter(param::factory::declareBool("boost/use_surrogates", false), use_surrogates_);
}
...
bool AdaBoostTrainer::processCollection(std::vector<FeaturesMessage>& collection)
{
    ...
    for (const FeaturesMessage& fm : collection) {
        ...
        if (fm.classification != 1 && fm.classification != -1)
    ...
    for (int i = 0; i < samples.rows; ++i) {
        ...
        for (int j = 0; j < samples.cols; ++j) {
            ...
            if (std::isnan(value) || std::isinf(value) || (fabs(value) >= 0.5f * FLT_MAX)) {
    ...
    cv::Ptr<cv::ml::TrainData> train_data_struct = cv::ml::TrainData::create(samples, cv::ml::ROW_SAMPLE, labels, cv::noArray(), cv::noArray(), cv::noArray(), cv::noArray());
