...
JANNLabMLP::JANNLabMLP() : mlp_input_size_(0), mlp_output_size_(0)
{
}
...
{
    addParameter(csapex::param::factory::declarePath("MLP_path", csapex::param::ParameterDescription("Path to a saved MLP."), true, ""), std::bind(&JANNLabMLP::load, this));

    addParameter(csapex::param::factory::declarePath("class_label_path", csapex::param::ParameterDescription("Path to a class label file."), true, ""), std::bind(&JANNLabMLP::load, this));

    addParameter(csapex::param::factory::declarePath("normalization_path", csapex::param::ParameterDescription("Path to a normalization file."), true, "", "*.norm"),
...
inline void convertNumeric(const std::vector<U>& src, std::vector<V>& dst)
{
    dst.resize(src.size());
        ...
        *out = *in;
        ...
        for (; it_in != in->end(); ++it_in, ++it_out) {
            if (it_in->value.size() != mlp_input_size_) {
                std::stringstream ss;
                ss << "Wrong feature count '" << it_in->value.size() << "', expected '" << mlp_input_size_ << "'!" << std::endl;
            ...
            *it_out = *it_in;
            ...
            if (it_in->type == FeaturesMessage::Type::CLASSIFICATION || config_.output_type == mlp::OutputType::SOFTMAX) {
            ...
            } else if (it_in->type == FeaturesMessage::Type::REGRESSION || config_.output_type == mlp::OutputType::LINEAR) {
...
void JANNLabMLP::load()
{
    ...
    //        node_modifier_->setWarning(std::string("Net has ") +
    //        std::to_string(connections) + " connections but " +
    //        std::to_string(weights_.size()) + " weights"); return;
    //    }
}
