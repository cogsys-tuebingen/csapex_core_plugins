...
namespace impl
{
inline void classify(const RandomTree& random_trees, const FeaturesMessage& in_feature, FeaturesMessage& out_feature, std::map<int, std::size_t>& votes)
{
...
RandomTrees::RandomTrees() : loaded_(false)
{
}

void RandomTrees::setupParameters(Parameterizable& parameters)
{
    parameters.addParameter(csapex::param::factory::declareFileInputPath("path", "rforest.yaml"), [this](param::Parameter* p) {
    ...
    parameters.addParameter(param::factory::declareBool("compute_class_weights", false), compute_class_weights_);
}
...
void RandomTrees::process()
{
    ...
    if (compute_class_weights_) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            impl::classify(random_trees_, input_feature->at(i), output_feature->at(i), class_labels);
