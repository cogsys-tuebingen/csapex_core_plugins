...
SVMEnsembleTrainer::SVMEnsembleTrainer() : rand_vec_(2)
{
}
...
void SVMEnsembleTrainer::setupParameters(Parameterizable& parameters)
{
    ...
    std::function<bool()> one_class_cond = [this]() { return svm_type_ != SVM::ONE_CLASS; };

    parameters.addConditionalParameter(param::factory::declareRange<double>("svm/degree", 0.0, 9.0, 3.0, 1.0), deg_cond, degree_);
    parameters.addConditionalParameter(param::factory::declareRange<double>("svm/gamma", 0.0, 10.0, 0.0, 0.01), gamma_cond, gamma_);
    parameters.addConditionalParameter(param::factory::declareRange<double>("svm/coef0", -10.0, 10.0, 0.0, 0.01), coeff0_cond, coef0_);
    parameters.addConditionalParameter(param::factory::declareRange<double>("svm/C", 0.0, 10.0, 0.01, 0.01), c_cond, C_);
    parameters.addConditionalParameter(param::factory::declareRange<double>("svm/nu", 0.0, 1.0, 0.5, 0.01), nu_cond, nu_);
    parameters.addConditionalParameter(csapex::param::factory::declareRange<double>("svm/p", 0.0, 1.0, 0.1, 0.01), p_cond, p_);
    parameters.addConditionalParameter(csapex::param::factory::declareBool("classes/one-vs-all", false), one_class_cond, one_vs_all_);

    parameters.addParameter(param::factory::declareBool("classes/balance", param::ParameterDescription("Use the same amount of samples per class."), false), balance_);

    parameters.addParameter(csapex::param::factory::declareValue<double>("svm_accuracy", csapex::param::ParameterDescription("Termination criteria of the iterative SVM training. Accuracy"), 1e-3),
                            epsilon_);
    ...
    if (svm_type_ != cv::ml::SVM::ONE_CLASS) {
        ...
        if (one_vs_all_) {
            ...
            for (std::size_t i = 0; i < indices_by_label.size(); ++i) {
                ...
                /// now we gather negative samples, which is basically collecting all
                /// the other classes
                for (const auto& entry : indices_by_label) {
                    ...
                    if (balance_) {
                        ...
                        neg_indices.insert(neg_indices.end(), entry.second.begin(), entry.second.end());
                    }
                }
