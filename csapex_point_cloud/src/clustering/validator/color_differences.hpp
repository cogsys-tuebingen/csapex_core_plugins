...
{
inline double CIE76(const Eigen::Vector3d& a, const Eigen::Vector3d& b, const std::array<double, 3>& weights)
{
    /// for lab
    /// delta_e > 2.3 noticeable difference
    return (Eigen::Vector3d((a(0) - b(0)) * weights[0], (a(1) - b(1)) * weights[1], (a(2) - b(2)) * weights[2])).norm();
}

inline double CIE94Grahpics(const Eigen::Vector3d& color_1, const Eigen::Vector3d& color_2, const std::array<double, 3>& weights)
{
...
inline double CIE94Textiles(const Eigen::Vector3d& color_1, const Eigen::Vector3d& color_2, const std::array<double, 3>& weights)
{
    const double dl = color_1(0) - color_2(0);
    const double C1 = hypot(color_1(1), color_1(2));
    const double C2 = hypot(color_2(1), color_2(2));
