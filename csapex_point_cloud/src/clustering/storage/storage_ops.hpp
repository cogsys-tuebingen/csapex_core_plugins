...
public:
    ...
    static void init(const pcl::PointCloud<PointT>& cloud, const pcl::PointIndices::ConstPtr& indices, const VoxelIndex& indexer, Storage& storage, std::vector<typename Storage::data_t>&,
    ...
    template <typename PointT, typename Storage>
    static void init(const pcl::PointCloud<PointT>& cloud, const pcl::PointIndices::ConstPtr& indices, const VoxelIndex& indexer, Storage& storage, std::vector<typename Storage::data_t>& data_storage,
                     std::true_type /*fixed_size*/)
    {
        ...
        storage.template set<cslibs_indexed_storage::option::tags::array_size>(static_cast<std::size_t>(size[0]), static_cast<std::size_t>(size[1]), static_cast<std::size_t>(size[2]));
    ...
    static void extract(const Storage& storage, const ClusterOp& cluster_op, std::vector<pcl::PointIndices>& accepted, std::vector<pcl::PointIndices>& rejected)
    {
