...
void TransformFromModels::process()
{
    ...
    // Process data
    StlVectorEigenVector3d points_ref = getInterestingPointsFromModels(models_ref);
    StlVectorEigenVector3d points_new = getInterestingPointsFromModels(models_new);
    ...
    // Check that there are 3 points for each model, so that we can match the
    // trinangles
    if ((points_ref.size() == 3) && (points_new.size() == 3)) {
        ...
        eulerAnglesFromRotationMatrix(rotation, roll, pitch,
                                      yaw);  // Convert Homogenious Coordinates to a Quaternion
...
void TransformFromModels::eulerAnglesFromRotationMatrix(const Eigen::Matrix3d& R, double& psi, double& theta, double& phi)
{
     ...
     *http://www.soi.city.ac.uk/~sbbh653/publications/euler.pdf but the second
     *solution is ignored Test the function with R =
     *[[ 0.5    -0.1464  0.8536]
     * [ 0.5     0.8536 -0.1464]
     * [-0.707   0.5     0.5   ]]
...
TransformFromModels::StlVectorEigenVector3d TransformFromModels::getInterestingPointsFromModels(std::shared_ptr<std::vector<ModelMessage> const> models)
{
    TransformFromModels::StlVectorEigenVector3d points;
    ...
    for (std::vector<ModelMessage>::const_iterator it = models->begin(); it != models->end(); it++) {
        ...
        switch (it->model_type) {
            ...
            case pcl::SACMODEL_CIRCLE2D: {
                interresting_point(0) = it->coefficients->values.at(0);
                interresting_point(1) = it->coefficients->values.at(1);
                // Calculate the z coordiantes of the apex, given the radius and the
                // opening angle
                double radius = it->coefficients->values.at(2);
                double tan_result = std::tan(param_cone_angle_ / 2.0);
                // apex_assert(tan_result != 0);
                double z;
                if (tan_result != 0) {
                    z = radius / tan_result;
                } else {
...
int TransformFromModels::matchSidesOfTriangles(const StlVectorEigenVector3d& points1, const StlVectorEigenVector3d& points2)
{
...
Eigen::Matrix4d TransformFromModels::calculateTransformation(const StlVectorEigenVector3d& points_ref, const StlVectorEigenVector3d& points_new, int offset)
{
    // Shift the reference points according to the offset
    StlVectorEigenVector3d points_new_shifted;
    ...
    return r_T_0 * n_T_0.inverse();  // orginal
                                     // return n_T_0 * r_T_0 .inverse(); //that the found transformation is the
                                     // same as the testone
}

using namespace std;

Eigen::Matrix4d TransformFromModels::threePointsToTransformation(const StlVectorEigenVector3d& points)
{
    ...
    if (points.size() > 2) {
        ...
        //        transformation.block<1,3>(0,0) = v1;          // x vector of the
        //        coordinate system transformation.block<1,3>(1,0) = v2;          //
        //        y vector of the coordinate system transformation.block<1,3>(2,0) =
        //        v1.cross(v2);          // z vector of the coordinate system
        //        transformation.block<1,3>(3,0) = points.at(0); // origin of the
        //        coordinate system transformation(3,3) = 1;
