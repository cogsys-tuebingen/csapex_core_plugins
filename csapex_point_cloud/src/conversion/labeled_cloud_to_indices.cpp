...
struct Impl
{
    inline static void extract(const typename pcl::PointCloud<PointT>::ConstPtr src, typename std::shared_ptr<std::vector<pcl::PointIndices>> dst)
    {
        // for(typename pcl::PointCloud<PointT>::const_iterator it = src->begin() ;
        // it != src->end() ; ++it) {
...
struct Conversion
{
    static void apply(const typename pcl::PointCloud<PointT>::ConstPtr src, typename std::shared_ptr<std::vector<pcl::PointIndices>> dst)
    {
...
{
    static void apply(const typename pcl::PointCloud<pcl::PointXYZL>::ConstPtr src, typename std::shared_ptr<std::vector<pcl::PointIndices>> dst)
    {
...
{
    static void apply(const typename pcl::PointCloud<pcl::PointXYZRGBL>::ConstPtr src, typename std::shared_ptr<std::vector<pcl::PointIndices>> dst)
    {
