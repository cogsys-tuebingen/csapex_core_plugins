...
public:
    ...
    void setupParameters(Parameterizable& parameters)
    {
        ...
        parameters.addConditionalParameter(param::factory::declareRange("search_radius", 0.01, 1.0, 0.03, 0.01), [this]() { return use_pca_; }, search_radius_);

        parameters.addConditionalParameter(param::factory::declareRange("max_depth_change_factor", 0.0, 0.5, 0.02, 0.001), [this]() { return !use_pca_; }, max_depth_change_factor_);
        parameters.addConditionalParameter(param::factory::declareRange("normal_smoothing_size", 0.0, 50.0, 10.0, 0.1), [this]() { return !use_pca_; }, normal_smoothing_size_);

        typedef pcl::IntegralImageNormalEstimation<pcl::PointXYZ, pcl::Normal> T;

        std::map<std::string, int> methods{ { "COVARIANCE_MATRIX", T::COVARIANCE_MATRIX },
        ...
        parameters.addConditionalParameter(param::factory::declareParameterSet("method", methods, (int)T::AVERAGE_3D_GRADIENT), [this]() { return !use_pca_; }, method_);
    }
    ...
    void inputCloud(typename pcl::PointCloud<PointT>::ConstPtr cloud)
    {
        ...
        if (use_pca_) {
            ...
            if (!cloud->isOrganized())
                throw std::runtime_error("Cannot use the chosen method to estimate "
                                         "normals - cloud must be organized!");
