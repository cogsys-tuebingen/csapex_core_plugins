...
{
    std::map<std::string, int> reference_axis = { { "x", (int)X }, { "y", (int)Y }, { "z", (int)Z } };

    addParameter(csapex::param::factory::declareParameterSet("reference axis", csapex::param::ParameterDescription("Choose the reference axis."), reference_axis, (int)X));
}
...
inline void processCloud(typename pcl::PointCloud<PointT>::ConstPtr cloud, const Eigen::Vector3d& ref_axis, cv::Mat& angles_general, cv::Mat& angles_component_a, cv::Mat& angles_component_b)
{
...
inline void processCloud(typename pcl::PointCloud<pcl::PointXY>::ConstPtr cloud, const Eigen::Vector2d& ref_axis, cv::Mat& angles_general)
{
    unsigned n = cloud->points.size();
...
struct Processor
{
    static void invokeProcess(RayAngles* instance, typename pcl::PointCloud<PointT>::ConstPtr cloud)
    {
...
{
    static void invokeProcess(RayAngles* instance, pcl::PointCloud<pcl::PointXY>::ConstPtr cloud)
    {
...
{
    static void invokeProcess(RayAngles* instance, pcl::PointCloud<pcl::PointNormal>::ConstPtr cloud)
    {
...
void RayAngles::doProcess3D(typename pcl::PointCloud<PointT>::ConstPtr cloud)
{
    ...
    impl::processCloud<PointT>(cloud, ref_axis, angles_general_, angles_component_a_, angles_component_b_);
}
...
void RayAngles::doProcess2D(pcl::PointCloud<pcl::PointXY>::ConstPtr cloud)
{
    ...
    impl::processCloud(cloud, ref_axis, angles_general_);
