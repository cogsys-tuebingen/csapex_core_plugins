...
public:
    ...
    {
        params.addParameter(param::factory::declareRange("distance xy", 0.0, 1.0, 0.3, 0.01), cluster_distance_xy_);
        params.addParameter(param::factory::declareRange("distance xy factor", 0.0, 10.0, 0.0, 0.01), cluster_distance_xy_distance_factor_);
        params.addParameter(param::factory::declareRange("distance xy offset", -3.0, 3.0, 0.0, 0.01), cluster_distance_xy_distance_offset_);
        params.addParameter(param::factory::declareRange("distance z", 0.0, 2.0, 0.3, 0.01), cluster_distance_z_);
        params.addParameter(param::factory::declareRange("max mean distance", 0.0, 3.0, 1.0, 0.01), cluster_max_mean_xyz_);

        params.addParameter(param::factory::declareRange("min cluster size", 0, 1000, 100, 1), cluster_min_size_);
            ...
            if (!c1.indices.empty()) {
                for (auto it2 = it + 1; it2 != merged_indices.end(); ++it2) {
                    ...
                    if (!c2.indices.empty()) {
                        ...
                        double factor = cluster_distance_xy_distance_factor_ == 0 ? 1.0 : cluster_distance_xy_distance_factor_ * d;
                        ...
                        if (distanceXYZ(mean1, mean2) < cluster_max_mean_xyz_) {
                            for (std::size_t k = 0, n = c1.indices.size(); k < n; ++k) {
                                ...
                                for (std::size_t l = 0, n = c2.indices.size(); l < n; ++l) {
                                    ...
                                    if (distanceXY(p1, p2) < cluster_distance_xy_ * factor && distanceZ(p1, p2) < cluster_distance_z_) {
                                        merge(c1, c2);
                                        mean[it] = calculateMean(cloud, c1);
                                        merged = true;
                                        break;
