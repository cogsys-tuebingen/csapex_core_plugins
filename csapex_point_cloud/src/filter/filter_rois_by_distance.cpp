...
{
    parameters.addParameter(param::factory::declareParameterSet<int>("method", { { "mean", MEAN }, { "median", MEDIAN } }, MEAN), method_);
    parameters.addParameter(param::factory::declareInterval("distance", -100.0, 100.0, 0.5, 5.0, 0.01), distance_);
    parameters.addParameter(param::factory::declareRange("min_point_count", 0, 100000, 0, 1), min_point_count_);
    parameters.addParameter(param::factory::declareBool("remove invalid", true), remove_);
}
...
inline bool check_filter(const RoiMessage& roi, const typename pcl::PointCloud<PointT>::ConstPtr& cloud, const std::pair<double, double>& distance, int min_point_count,
                         FilterROIsByDistance::Method method)
{
    ...
    if (method == FilterROIsByDistance::Method::MEAN) {
        ...
        bool constraints_apply = (int)valid_points > min_point_count && mean_depth >= distance.first && mean_depth <= distance.second;
        ...
        if (depths.empty())
            ...
            bool constraints_apply = (int)depths.size() > min_point_count && median_depth >= distance.first && median_depth <= distance.second;
