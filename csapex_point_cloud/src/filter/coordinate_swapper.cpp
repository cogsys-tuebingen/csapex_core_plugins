...
inline static void swap(PointT& p1, typename std::enable_if<PointTraits<PointT>::HasXYZ, PointT>::type* dummy = 0)
{
    PointT tmp = p1;
...
struct Swapper
{
    inline static void apply(const typename pcl::PointCloud<PointT>::ConstPtr& src, typename pcl::PointCloud<PointT>::Ptr& dst)
    {
