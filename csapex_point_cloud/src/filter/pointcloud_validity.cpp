...
inline static bool invalid(const PointT& p, typename std::enable_if<PointTraits<PointT>::HasXY, PointT>::type* dummy = 0)
{
...
template <typename PointT>
inline static bool invalid(const PointT& p, typename std::enable_if<PointTraits<PointT>::HasXYZ, PointT>::type* dummy = 0)
{
...
struct Vadility
{
    inline static void inceces(const typename pcl::PointCloud<PointT>::ConstPtr& src, std::vector<int>& indices)
    {
    ...
    inline static void mask(const typename pcl::PointCloud<PointT>::ConstPtr& src, cv::Mat& mask)
    {
        const PointT* src_ptr = src->points.data();
        int cols = src->width;
        int rows = src->height;
