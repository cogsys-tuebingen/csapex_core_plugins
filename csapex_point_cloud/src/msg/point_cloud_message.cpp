...
PointCloudMessage::PointCloudMessage(const std::string& frame_id, Message::Stamp stamp) : Message(type<PointCloudMessage>::name(), frame_id, stamp)
{
}

PointCloudMessage::PointCloudMessage() : Message(type<PointCloudMessage>::name(), "/", 0)
{
}
...
struct Import
{
    Import(const YAML::Node& node, PointCloudMessage::variant& value, const std::string& type) : node(node), value(value), type(type)
    {
    }
...
{
    Export(YAML::Node& node) : node(node)
    {
    }
